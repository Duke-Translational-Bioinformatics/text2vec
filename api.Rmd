...
Goals wich we aim during the development of `text2vec`:

* **Consise** - expose as few functions as possible
* **Consistent** - expose unified interfaces, no need to learn new interface for each task
* **Flexible** - allow to easlily solve complex tasks
* **Fast** - maximize efficiency per single thread, transparently scale to multiple threads on multicore machines
* **Memory efficient** - use streams and iterators, don't keep data in RAM if possible
...
## I/O handling. 
All functions from `create_*` family work with **iterators** over tokens as input. Functions are good examples for creation of such iterators. Once user need some custom source, he/she just need to create correct iterator over tokens. Good examples are:
...
**All text2vec models are mutable! This means that `fit()` and `fit_transform()` methods change model which fed as argument.**
...
* `model$fit_transform(x, ...)` - fits model to data and then transform data.
* `model$transform(x_new, ...)` - transorms new data with pretrained model.

## Distances 

See [Distances](distances.html) section for details.

text2vec provides only **two** functions for calculation distances between 2 sets of the documents, assuming each set of documents is represented as (sparse)matrix:

* `dist2(x, y, method)` - calculates distance between **each row** of matrix `x` and **each row** of matrix `y` using given `method`.
* `pdist2(x, y, method)` - calculates **p**arallel distance between rows of matrix `x` and **corresponding** rows of matrix `y` using given `method.` 
