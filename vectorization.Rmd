...
Note that we use exactly the same functions from preprosessing and tokenization. Also we reuse/use the same `vectorizer` - function which maps terms to indices.
...
Seems that usage of n-grams improved our model a little bit more. Lets check performance on test dataset:
